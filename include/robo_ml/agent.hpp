...
#include <robo_ml_ros/actor.hpp>
#include <robo_ml_ros/observer.hpp>
...
public:
  ...
  : Node("agent", options)
  {
    ...
    const auto obs_from_srcs_callback = [this](std::shared_ptr<SrcTs>... src_msgs) {
        return obsFromSrcs(src_msgs ...);
      };
    observer_ = std::make_shared<Observer<ObsT, SrcTs...>>(options, obs_from_srcs_callback);
    actor_ =
      std::make_shared<Actor<ActionT>>(
      options,
      std::bind(&Agent::actionFromObs, this, observer_));
  }
   ...
   */
  virtual typename ActionT::Feedback::SharedPtr actionFromObs(
    std::shared_ptr<Observer<ObsT,
    SrcTs...>> observer) = 0;
