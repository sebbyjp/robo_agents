...
public:
    ...
    , last_src_msgs_recieved_(std::make_tuple(std::make_shared<SrcTs>()...))
  {
    ...
    src_subs_ = dgl::util::addSubscriptions(
      this, src_topics, src_sub_group, last_src_msgs_recieved_,
      ...
      std::chrono::milliseconds(100), [this]() -> void {
        if (observation_cache_.size() > 0 && this->get_parameter("publish_observation").as_bool()) {
          ...
          try {
            ...
            RCLCPP_WARN(
              this->get_logger(),
              "Tried to update observation timestamp. Exception caught: %s", e.what());
          }
...
private:
  ...
  {
    return std::all_of(
      recieved_first_src_msgs_.begin(), recieved_first_src_msgs_.end(), [](bool b) {
        return b;
      });
  }
